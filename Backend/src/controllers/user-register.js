...
export const register = async ( req, res, next ) => {
  try {
    ...
    if ( !avatarpath ) {
      throw new ApiError( 406, "Unable to upload Avatar on cloud, Please try again" );
    }
...
export const login = async( req, res, next ) =>{
  ...
  try {
    ...
    const{ generatedrefreshToken, generatedaccessToken } = await generateRefreshTokenAndAccessToken( user._id );
